...
class PuzzleGenerator implements PuzzleSource {
            ...
            if (sq.successor() == null && sq.direction() != 0) {
                ...
                if (sq.sequenceNum() != 0) {
                    if (sq.successors() != null) {
                        for (Place x : sq.successors()) {
                            Sq b = model.get(x);
                            int c = model.get(x).sequenceNum();
                            if (sq.connectable(b) && c != 0) {
                                found = model.get(x);
                                nFound = 1;
                            }
                        }
    ...
    private int makeBackwardConnections(Model model) {
        ...
        for (Sq sq : model) {
            ...
            if (sq.predecessor() == null && sq.sequenceNum() != 1) {
                ...
                if (sq.predecessors() != null && sq.sequenceNum() != 0) {
                    for (Place x: sq.predecessors()) {
                        boolean d = connP(model, x, sq);
                        int e = model.get(x).sequenceNum();
                        if (d && e != 0) {
                            found = model.get(x);
                            nFound = 1;
                        }
    ...
    /** @return boolena connectable
     * @param model ,
     * @param p ,
     * @param sq .*/
    public boolean connP(Model model, Place p, Sq sq) {
        return model.get(p).connectable(sq);
    }
