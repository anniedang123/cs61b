...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int k = 1; k <= last; k++) {
            if (_solnNumToPlace[k] == null) {
                throw new IllegalArgumentException("SeqNum must be 1-last");
            }
        }
        for (int x0 = 0; x0 < _width; x0++) {
            for (int y0 = 0; y0 < _height; y0++) {
                int sSeqNum = solution[x0][y0], group = -1, dir = 0, seqNum = 0;
                boolean fixed = false;
                if (sSeqNum == 1 || sSeqNum == last) {
                    fixed = true;
                }
                if (fixed) {
                    group = 0;
                ...
                if (seqNum != last) {
                    Place pl1 = _solnNumToPlace[sSeqNum];
                    Place pl2 = _solnNumToPlace[sSeqNum + 1];
                    dir = Place.dirOf(pl1.x, pl1.y, pl2.x, pl2.y);
                }
                _board[x0][y0] = new Sq(x0, y0, seqNum, fixed, dir, group);
            }
        }
        for (Sq[] col: _board) {
            for (Sq sq: col) {
        ...
        for (Sq sq : _allSquares) {
            ...
            for (Place p : sq._successors) {
                if (_board[p.x][p.y]._predecessors == null) {
                    PlaceList nulPl = new PlaceList();
                    _board[p.x][p.y]._predecessors = nulPl;

                }
                _board[p.x][p.y]._predecessors.add(Place.pl(sq.x, sq.y));
            }
        }
        _unconnected = last - 1;
    }
    ...
    Model(Model model) {
        ...
        for (Sq sq : model._allSquares) {
            Sq sq0 = get(sq.x, sq.y);
            if (sq.successor() != null) {
                Sq s = sq.successor().successor();
                sq0 = sq0.successor();
                Sq sCop = get(s.x, s.y);
                sq0._successor = sCop;
            }
            sq0 = get(sq.x, sq.y);
            if (sq.successor() != null) {
                Sq p = sq.successor().predecessor();
                sq0 = sq0.predecessor();
                Sq pCop = get(p.x, p.y);
                sq0._successor = pCop;
            }
            sq0._head = get(sq.head().x, sq.head().y);
        }
    }
        ...
        for (Sq[] col : _board) {
            for (Sq sq : col) {
                if (sq.sequenceNum() != 0 && sq.successor() == null) {
                    for (Sq r : _allSquares) {
                        boolean o = sq.connectable(r);
                        boolean p = r.sequenceNum() == sq.sequenceNum() + 1;
                        if (o && p) {
                            sq.connect(r);
                            return true;
                        }
    ...
    private int arrowDirection(int x, int y) {
        ...
        Place succPl = solnNumToPlace(seq0);
        Sq succ = get(succPl)._successor;
        if (succ != null) {
            return Place.dirOf(succPl.x, succPl.y, succ.x, succ.y);
        }
        return 0;
    }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            boolean gDir = this.pl.dirOf(s1.pl) == this.direction();
            boolean gSN = s1.sequenceNum() - 1 == this.sequenceNum();
            boolean diffGroup = (this.head().group() != s1.head().group());
            boolean is = (this.head().group() == -1 && s1.head().group() == -1);
            int r = this.sequenceNum();
            int p = s1.sequenceNum();
            if ((r != 0 && p == 0) || (r == 0 && p != 0)) {
                return s1.predecessor() == null && gDir;
            } else if (this.sequenceNum() != 0 && s1.sequenceNum() != 0) {
                return s1.predecessor() == null && gDir && gSN;
            } else if (r == 0 && p == 0 && !diffGroup) {
                return true;
            }
            return gDir && (diffGroup || is);
        }
        ...
        public boolean equals(Object obj) {
            ...
            return sq != null
                    ...
                    && (p == null || _successor.pl == sq._successor.pl);
        }
